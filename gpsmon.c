...
static void usage(void)
{
	(void)fputs(
		 ...
		 "  -p				  PostgreSQL connection string\n"
		 "  -s				  Serial mane for measurments\n",
		 stderr);
}
...
int main(int argc, char **argv)
{
	...
	int activated = -1;
	const char *optstring = "?p:s:aD:hLl:nt:V";
#ifdef HAVE_GETOPT_LONG
	int option_index = 0;
	static struct option long_options[] = {
		...
		{"pg_conn", required_argument, NULL, 'p'},
		{"pg_seria", required_argument, NULL, 's'},
	...
	while (1) {
		...
		case 'p':
			{
				char *postgres_conn_str = optarg;
				printf("postgres_conn_str %s ", postgres_conn_str);
				/* Соединяемся с БД */
				pg_conn = PQconnectdb(postgres_conn_str);
				...
				printf ("PostgreSQL DB OK");
			}
			break;
		case 's':
			postgres_seria_str = optarg;
			printf("postgres_seria_str %s", postgres_seria_str);
			break;
		case 'a':
			nocurses = true;
			break;
